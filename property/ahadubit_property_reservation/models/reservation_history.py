...
class Property(models.Model):
    ...
    reservation_end_date=fields.Datetime(string="Reservation End Date", compute="compute_reservation_end_date")

    @api.depends('reservation_line_ids')
    def compute_reservation_end_date(self):
        for rec in self:
            reservation=self.env['property.reservation'].search([('property_id', '=', rec.id),('status', '=', 'reserved')],limit=1)
            if reservation:
                rec.reservation_end_date=reservation.expire_date
            else:
