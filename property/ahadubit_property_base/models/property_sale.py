...
class PropertySale(models.Model):
    ...
    payment_commission_line_ids=fields.One2many('property.commission.line',  compute='_compute_payment_commission_line_ids',inverse_name='sale_id',store=True)
    ...
    def _compute_payment_commission_line_ids(self):
        for sale in self:
            # Clear existing commission lines to ensure synchronization
            sale.payment_commission_line_ids.unlink()

            # Create new commission lines based on installment lines
            commission_lines = []
            total_commission = sale.sale_price * (sale.property_id.commision_percent / 100)

            for installment_line in sale.payment_installment_line_ids:
                commission_lines.append({
                    'sale_id': sale.id,
                    'payment_term_id': installment_line.payment_term_id.id,
                    'expected': installment_line.expected, # Same percentage as installment line
                    'expected_amount': installment_line.expected_amount * (sale.property_id.commision_percent / 100), # Same percentage as installment line

                    'paid_amount': installment_line.paid_amount * (sale.property_id.commision_percent / 100),  # Proportional to commission percent
                    'discount': installment_line.discount * (sale.property_id.commision_percent / 100),  # Proportional to commission percent
                    'expected_date': installment_line.expected_date,  # Same expected date
                })

            # Create commission lines
            if commission_lines:
                self.env['property.commission.line'].create(commission_lines)
