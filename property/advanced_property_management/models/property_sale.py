...
class PropertySale(models.Model):
    ...
    total_commission=fields.Float(string="Total Commission",related="property_id.sales_commission")

    paid_commission=fields.Float(string="Paid Commission",compute="_compute_paid")
    remaining_commission=fields.Float(string="Remaining Commission",compute="_compute_remaining")

   
    @api.depends('remaining_commission')

    def _compute_paid(self):
        for record in self:
            commision_percent=record.property_id.commision_percent
            record.paid_commission=record.total_paid*commision_percent/100

    @api.depends("paid_commission","total_commission")
